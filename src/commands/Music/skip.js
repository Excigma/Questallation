...
module.exports = class extends MusicCommand {
	...
	async run(msg, [force]) {
		...
		if (music.voiceChannel.members.size > 3) {
			if (force) {
				if (!await msg.hasAtLeastPermissionLevel(5)) return msg.send("<:excigmabot_failure:490319592477032448> | You can't force skip unless you have manage server");
			} else {
		...
		await msg.sendMessage(`<:excigmabot_success:490319592615575553> | Skipped ${music.queue[0].title}`);
		music.skip(true);
		return null;
	}
