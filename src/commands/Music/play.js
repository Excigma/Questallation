...
module.exports = class extends MusicCommand {
	...
	async run(msg) {
		...
		if (music.playing) {
			...
			music.resume();
			return msg.sendMessage(`<:excigmabot_success:490319592615575553> | Resumed, now playing: **${music.queue[0].title}**`);
		} else {
	...
	async play(music) {
		while (music.queue.length) {
			...
			await music.channel.send(`<:excigmabot_success:490319592615575553> | Playing: **${song.title}** as requested by: **${song.requester}**`);
			await sleep(300);
			...
			try {
				if (!await new Promise(async (resolve) => {
					(await music.play())
						...
						.on("error", (err) => {
							music.channel.send("<:excigmabot_failure:490319592477032448> | An unexpected error occured");
		...
		if (!music.queue.length) {
			music.channel.send("<:excigmabot_success:490319592615575553> | There are no more songs, I will now leave the voice channel")
				.then(() => music.leave());
		}
	}
