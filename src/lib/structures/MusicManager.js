...
class MusicManager {
    ...
    async play() {
        ...
        if (!this.connection) {
            await this.channel.send(`I'm not connected to a voice channel. Leaving and joining back...`)
                .catch(error => this.client.emit("error", error));

            const { voiceChannel } = this;
            this.forceDisconnect();
            await this.join(voiceChannel);
            if (!this.connection) throw "There is something wrong";
        }
        ...
        const stream = ytdl(`https://youtu.be/${song.url}`, {
            filter: song.opus ?
                format => format.type === "audio/webm; codecs=\"opus\"" : "audioonly"
